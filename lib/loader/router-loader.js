...
function route(router, conf) {
    ...
    for (var key in conf.routes) {
        ...
        if (isApi) {
            ...
            var validSafeCode = false
            if (typeof(conf.validate_safe_code) !== 'undefined' && conf.validate_safe_code) {
                validSafeCode = true
            }
            if (validSafeCode) {
                var validSafeCodeMiddleware = require('../middleware/valid-safe-code-middleware')
                router.use(apiPath, validSafeCodeMiddleware())
            }
            //support filter 
