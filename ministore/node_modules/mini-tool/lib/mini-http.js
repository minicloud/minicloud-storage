...
function MiniHttp(appConfig,req,res,logger){ 
	...
	for (var key in queryParams) {
		if(queryParams.hasOwnProperty(key)){
			params[key] = self.decode(queryParams[key]);
		}
    };
    ...
    form.parse(req, function(err, fields, files) { 
    	for (var key in fields) {
    		if(fields.hasOwnProperty(key)){
    			params[key] = self.decode(fields[key]);
    		}
    	}
        //兼容老版iOS文件上传
        if(typeof(files.file)=="undefined" && 
        	(typeof(files.files)!='undefined' && typeof(files.files.path)!='undefined')){
           self.files = {};
           self.files.file = {path:files.files.path};
        }else{
          self.files = files;
        }
    	self.params = params;
...
* 解码
*/
MiniHttp.prototype.decode=function(str){
	try{  
...
MiniHttp.prototype.xsendFile=function(nginxPath,fileName,mimeType,forceDownload){
	var userAgent = (this.req.headers['user-agent']||'').toLowerCase();
	var options = {
	...
	if(userAgent=="" || userAgent.indexOf('trident') >= 0||userAgent.indexOf('msie') >= 0 || userAgent.indexOf('chrome') >= 0 || userAgent.indexOf('miniclient') >= 0) {
		//ie+chrome浏览器
		...
		var encodeName = iconv.convert(fileName).toString();
		options["Content-Disposition"] = disposition+'; filename="' + encodeName + '"' ;
	}else {
		/* 其他非主流浏览器只能自求多福了*/
